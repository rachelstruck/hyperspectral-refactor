...
def set_wavelength_range(df, a,b):
    df_new = pd.DataFrame({'wavelength':range(a,b), 'excitation':[0.]*(b-a), 'emission':[0.]*(b-a)}, index=range(a,b))
    ...
    for wavelength in range(a,b):
        if wavelength in wavelength_list:
            ...
            df_new.loc[wavelength:wavelength, ('excitation', 'emission')] = [excitation, emission]
    return df_new

# reads and processes fluorophore data file
def load_fluorophore(name, filename=None, excitation_column_name=None, emission_column_name=None, index_range=(400,900)):
    if filename == None:
        filename = "fluorophore-spectra/" + name.replace('.','p') + "_fpbase_spectra.csv"
    ...
    df_columns = df.rename(columns ={name+' ex': 'excitation', name+' em': 'emission'})
...
class Fluorophore():
    def __init__(self, name, brightness, filename=None, excitation_column_name=None, emission_column_name=None, index_range=(400,900)):
        self.spectra = load_fluorophore(
            name, filename=filename, excitation_column_name=excitation_column_name,
            emission_column_name=emission_column_name, index_range=index_range
            )
        self.name = name
...
def get_spectra_list(wavelength_range=(400,900)):
    ...
    for fluorophore_name in fluorophore_string_list:
        ...
        spectra_list.append(load_fluorophore(fluorophore_name, index_range=wavelength_range))
    return spectra_list
